...
def test_lens_path_length_point_rays_object_to_image_plane(paraxial_point_rays):
    ...
    def opl_lens(x1, y1, u1, v1, x2, y2, z1, z2):
        ...
        opl1 = np.sqrt((u1 - x1) ** 2 + (v1 - y1) ** 2 + z1**2) + np.sqrt(
            (u1 - x2) ** 2 + (v1 - y2) ** 2 + z2**2
        )

        # The lens should apply the path length different to each actual ray,
        # such that all rays will have the same opl
    ...
    opl_before_lens_temgym = (
        lens_rays.path_length.copy()
    ...
    opl_shift_lens_temgym = (
        opl_after_lens_temgym - opl_before_lens_temgym
    )  # Calculate the path length temgyms lens applies to the ray
    opl_image_lens_temgym = (
        rays_at_image.path_length
    )  # Get the path length at the image plane from temgym

    # Calculate the path length of the lens analytically,
    # with almost no approximations (Only the approximation that the rays
    # leave the lens from the same height they entered from - i.e the abbe sine condition in the
    # small angle approximation)
    opl_before_lens_analytic = opl_object + np.sqrt(
        object_plane**2
        + (lens_rays.x - input_rays.x) ** 2
        + (lens_rays.y - input_rays.y) ** 2
    )
    opl_shift_lens_analytic = opl_lens(
        input_rays.x,
        input_rays.y,
        lens_rays.x,
        lens_rays.y,
    ...
    opl_image_lens_analytic = opl_after_lens_analytic + np.sqrt(
        image_plane**2
        + (lens_rays.x - rays_at_image.x) ** 2
        + (lens_rays.y - rays_at_image.y) ** 2
    )

    xp.testing.assert_allclose(
        opl_before_lens_temgym, opl_before_lens_analytic, atol=1e-6
    )
    xp.testing.assert_allclose(
        opl_shift_lens_temgym, opl_shift_lens_analytic, atol=1e-6
    )
    xp.testing.assert_allclose(
        opl_after_lens_temgym, opl_after_lens_analytic, atol=1e-6
    )
    xp.testing.assert_allclose(
        opl_image_lens_temgym, opl_image_lens_analytic, atol=1e-6
    )


def test_lens_path_length_point_rays_from_focal_plane_to_veryfaraway(
    paraxial_point_rays,
):
    # Small analytic function to test optical path length of lens
    def opl_lens(x1, y1, u1, v1, x2, y2, z1, z2):
...
def test_double_deflector_deflection(parallel_rays):
    ...
    double_deflector = comp.DoubleDeflector(
        first=comp.Deflector(
            z=parallel_rays.location, defx=defx1, defy=defy1, name="Upper"
        ),
        second=comp.Deflector(
            z=parallel_rays.location + separation, defx=defx2, defy=defy2, name="Lower"
        ),
    )
...
def test_biprism_total_deflection_is_one(point_rays):
    ...
    xp.testing.assert_allclose(
        total_deflection, xp.ones(point_rays.num) * xp.abs(deflection)
    )
...
def test_aperture_blocking(parallel_rays, empty_rays):
    aperture = comp.Aperture(z=parallel_rays.location, radius=0.0)
...
def test_aperture_nonblocking(parallel_rays):
    aperture = comp.Aperture(z=parallel_rays.location, radius=2.0)
    out_rays = tuple(aperture.step(parallel_rays))[0]
